/*=========================================================================
  ...
  Module:    TestSeedWidgetOrientedHandleTextMany.cxx
...
int TestSeedWidgetOrientedHandleTextMany( int argc, char *argv[] )
{
  bool testText = false;
  for (int c = 0; c < argc; c++)
    {
      //std::cout << c << " " << argv[c] << std::endl;
  ...
  handleRep3->SetUniformScale(100.0);
  handleRep3->Modified();
  handleRep3->GetProperty()->SetColor(1,1,0);
  if (testText)
    {
      handleRep3->SetLabelVisibility(1);
      handleRep3->SetLabelText( "Seed-2-big" );
    }
  //handleRep3->SetUniformScale(40.0);
  //handleRep3->Modified();
  std::cout << "Set sides to 100\n" << std::endl;
  ...
  while( x < maxX)
      ...
      while (y < maxY)
	{
	  ...
	  while (z < maxZ)
	    {  
	      vtkHandleWidget * handleWidgetN = widget->CreateNewHandle();
	      handleWidgetN->SetEnabled(1);
	      // double posN[3] = { vtkMath::Random(-1.0, 1.0),
	      //			 vtkMath::Random(-1.0, 1.0),
	      //			 vtkMath::Random(-1.0, 1.0) };
	      double posN[3] = {x, y, z};
	      vtkOrientedPolygonalHandleRepresentation3D *rep = 
		vtkOrientedPolygonalHandleRepresentation3D::SafeDownCast(handleWidgetN->GetRepresentation());
	      if (rep)
		{
		  rep->SetWorldPosition(posN);
		  if (testText)
		    {
